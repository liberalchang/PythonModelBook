...
变量是编程的基础概念，用于存储和操作数据。Python提供了丰富的数据类型，包括数字、字符串、布尔值等基本类型，以及列表、字典等复合类型。
...
| 不能是关键字 | 不能使用Python保留字 | 不能用 `if`, `for` 等 |
...
print(message)  # 输出: 我是Alice，今年25岁
```
...
- **可变vs不可变**: 字符串、数字、元组是不可变的；列表、字典、集合是可变的
- **变量作用域**: 注意全局变量和局部变量的区别
- **内存管理**: Python自动管理内存，但要注意循环引用
...
- [控制流程](control-flow.md) - 学习如何使用变量进行条件判断
- [函数定义](functions.md) - 了解变量在函数中的作用域
- [内置函数](../builtins/index.md) - 学习操作不同数据类型的内置函数
