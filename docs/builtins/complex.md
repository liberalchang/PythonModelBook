...
`complex()` 是 Python 中的内置函数，用于创建复数对象或将其他数值类型转换为复数。复数是数学中的重要概念，由实部和虚部组成，在科学计算、信号处理、工程计算等领域有广泛应用。Python 原生支持复数运算，使得复数计算变得简单直观。

## 🎯 学习目标

- 掌握 complex()函数的基本用法和语法
- 理解复数的概念和表示方法
- 学会复数的基本运算和操作
- 了解复数在实际编程中的应用场景
- 掌握复数与其他数值类型的转换
...
print(f"repr 表示: {repr(c)}")  # 输出: (3+4j)
```
...
L = circuit.add_inductor(0.1, frequency)  # 0.1H 电感
C = circuit.add_capacitor(100e-6, frequency)  # 100μF 电容
...
## 计算电流和功率
voltage = complex(220, 0)  # 220V 交流电压
...
## H-Z-H 序列
qubit.hadamard()
print(f"第一个 H 门后: {qubit}")

qubit.pauli_z()
print(f"Z 门后: {qubit}")

qubit.hadamard()
print(f"第二个 H 门后: {qubit}")
...
- [Python 官方文档 - complex()](https://docs.python.org/3/library/functions.html#complex)
- [Python 官方文档 - 数值类型](https://docs.python.org/3/library/stdtypes.html#numeric-types-int-float-complex)
- [Python 官方文档 - cmath 模块](https://docs.python.org/3/library/cmath.html)
