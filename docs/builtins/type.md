...
`type()` 是Python中的内置函数，具有两种主要用法：作为单参数函数时返回对象的类型，作为三参数函数时可以动态创建新的类型对象。它是Python类型系统和元编程的重要工具。<mcreference link="https://docs.python.org/3/library/functions.html" index="1">1</mcreference>

## 🎯 学习目标

- 掌握type()函数的两种用法
- 理解type()与isinstance()的区别
- 学会使用type()进行类型检查
- 了解动态创建类的方法
...
# 正确调用
print(greet("张三", 25))  # 你好，张三，你今年25岁
...
book = Book(title="Python编程", author="张三", price=59.9)
print(book)  # Book(title=Python编程, author=张三, price=59.9)
print(book.to_dict())  # {'title': 'Python编程', 'author': '张三', 'price': 59.9}
```
...
if type(child) == Parent:
    print("是Parent类型")  # 这不会执行

# 正确的类型检查方式（推荐）
if isinstance(child, Parent):
    print("是Parent类型或其子类")  # 这会执行
...
- [Python官方文档 - type()](https://docs.python.org/3/library/functions.html#type)
- [Python数据模型 - 元类](https://docs.python.org/3/reference/datamodel.html#metaclasses)
- [Python类型系统详解](https://docs.python.org/3/library/typing.html)
