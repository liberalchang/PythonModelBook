...
`reversed()` 是Python中的内置函数，用于返回一个反转的迭代器。它可以处理序列（如列表、元组、字符串）或实现了`__reversed__()`方法的对象。<mcreference link="https://docs.python.org/3/library/functions.html" index="1">1</mcreference>
...
# print(next(rev_iter))  # StopIteration异常
...
def demonstrate_lazy_evaluation():
    ...
    print(f"创建reversed迭代器: {type(rev_iter)}")
...
print(f"最近3个数据点: {list(zip(recent_times, recent_vals))}")
...
- 字典在Python 3.7+中保持插入顺序，可以使用 `reversed()`
...
# 1. 不支持reversed()的对象
try:
    result = reversed({1, 2, 3})  # 集合不支持reversed()
...
- [Python官方文档 - reversed()](https://docs.python.org/3/library/functions.html#reversed)
